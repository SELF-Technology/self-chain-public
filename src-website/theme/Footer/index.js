...
function FooterWrapper(props) {
  ...
  React.useEffect(() => {
    ...
    if (footerThemeToggleWrapper) {
      // Clear any existing button first
      footerThemeToggleWrapper.innerHTML = '';
      const button = document.createElement('button');
      button.className = 'footer-theme-toggle';
      button.setAttribute('aria-label', 'Toggle theme');
      // Title will be updated by updateButton function
      
      const updateButton = () => {
        const currentMode = document.documentElement.getAttribute('data-theme') || 'light';
        button.setAttribute('title', `Switch to ${currentMode === 'dark' ? 'light' : 'dark'} mode`);
