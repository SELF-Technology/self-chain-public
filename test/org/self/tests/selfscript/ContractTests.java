package org.self.tests.selfscript;
...
//        }

        {
            ...
            assertEquals("org.self.selfscript.exceptions.SelfParseException: Invalid Token where there should be a COMMMAND - lorem", ctr.getException());
            assertTrue(ctr.getCompleteTraceLog().length() > 0);
        }
    }
    ...
    public void testExecution() {
...

        {
            ...
            {
            	SelfLogger.log("RUN THIS.. ");
                ctr.run();
                assertTrue(ctr.isParseOK());
                assertTrue(ctr.isException());
                assertEquals("org.self.selfscript.exceptions.ExecutionException: MAX instruction number reached! 1025", ctr.getException());
            }
        }
