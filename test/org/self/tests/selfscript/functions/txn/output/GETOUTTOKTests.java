...
//HEXValue GETOUTTOK (NumberValue input)
public class GETOUTTOKTests {

    @Test
    public void testConstructors() {
        GETOUTTOK fn = new GETOUTTOK();
        SelfFunction mf = fn.getNewFunction();

        assertEquals("GETOUTTOK", mf.getName());
        assertEquals(0, mf.getParameterNum());

        try {
            mf = SelfFunction.getFunction("GETOUTTOK");
            assertEquals("GETOUTTOK", mf.getName());
            assertEquals(0, mf.getParameterNum());
