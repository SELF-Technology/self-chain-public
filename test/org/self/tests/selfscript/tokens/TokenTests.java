...
public class TokenTests {
    ...
    public void testConstructors() {
        ...
        for (SelfFunction f : SelfFunction.ALL_FUNCTIONS) {
...
//        assertTrue(Token.isNumeric("-1"));
    ...
    public void testTokenize() {
...

        {
            ...
            try {
                List<ScriptToken> tokens = ScriptToken.tokenize(Script);
                for (int i = 0; i < SelfFunction.ALL_FUNCTIONS.length; i++) {
                    assertEquals(ScriptToken.TOKEN_FUNCTIION, tokens.get(i).getTokenType());
                    assertEquals(SelfFunction.ALL_FUNCTIONS[i].getName(), tokens.get(i).getToken());
                }
            } catch (SelfParseException ex) {
                fail();
            }
...
//            assertThrows(SelfParseException.class, () -> { // should throw this
//                Token.tokenize(Script);
//            });
//        }
    }
