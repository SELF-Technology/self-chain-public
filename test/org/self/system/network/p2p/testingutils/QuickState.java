package org.self.system.network.p2p.testingutils;
...
public class QuickState {
    ...
    public static P2PState stateNoConnections(boolean hasSelfAddress) throws UnknownHostException {
        P2PState state = new P2PState();
        state.getKnownPeers().addAll(QuickInetLists.generateInetSockAddrList("192.168.0.", 1, 9001));
        if (hasSelfAddress) {
            state.setMySelfAddress("10.0.0.1");
