	...
	public void PostMessage(TimerMessage zMessage) {
		synchronized (mMessagesLock) {
			if(zMessage != null) {
				...
				//Intermittent Bug..
				SelfLogger.log("NULL TIMER Message attempt:");
	...
	public void printAllMessages() {
		synchronized (mMessagesLock) {
			...
			for(TimerMessage tm : mTimerMessages) {
				SelfLogger.log(tm.toString(),false);
			}
		}
	}
		...
		while(mRunning) {
			...
			synchronized (mMessagesLock) {
				...
				for(TimerMessage tm : mTimerMessages) {
					...
					if(tm == null) {
						SelfLogger.log("Timer Message is NULL.. ?");
						continue;
					}
					...
					if(tm.getTimer()<time) {
						...
						if(process.isRunning()) {
							...
							SelfLogger.log("Timer Message NOT run as processor shutdown.. "+tm.toString());
						}
					}else {
						//Keep for next test
						newlist.add(tm);
