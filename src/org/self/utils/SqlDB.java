...
public abstract class SqlDB {
	...
	public void loadDB(File zFile) throws SQLException {
		...
		if(Main.STARTUP_DEBUG_LOGS) {
			SelfLogger.log("SQLDB Connect.. "+h2db);
		}
		...
		if(Main.STARTUP_DEBUG_LOGS) {
			SelfLogger.log("SQLDB Connected.. "+h2db);
		}
	...
	public boolean checkOpen(boolean zLogs) throws SQLException {
		...
		if(reopen) {
			...
			//Notify User
			if(zLogs) {
				SelfLogger.log("SqlDB requires re-open : "+mSQLDBNoMV.getName());
			}
	...
	public void saveDB(boolean zCompact) {
		try {
			...
			//Check..
			if(mSQLConnection == null) {
				SelfLogger.log("Trying to saveDB on NULL SQLDB");
				return;
			}
	...
	public synchronized JSONObject executeGenericSQL(String zSQL) {
		...
		try {
			...
			SelfLogger.log("ExecuteSQL sql:"+zSQL+" error:"+e.toString(),false);
