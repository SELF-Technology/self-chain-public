...
public class maxextra extends Command {
	...
	public String getFullHelp() {
		return "\nmaxextra\n"
				...
				+ "Examples:\n"
				+ "\n"
				+ "maxextra action:staticmls host:Sx...@34.190.784.3:9001\n"
				+ "\n"
				...
				+ "\n"
				+ "maxextra action:getaddress maxaddress:MAX#0x3081..#Sx..@34.190.784.3:9001\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		if(action.equals("addpermanent")) {
			...
			if(!host.equals("clear") && !checkValidSxAddress(host)) {
				throw new CommandException("Invalid MLS address : MUST be of type Sx..@host:port");
			}
	...
	public static MLSPacketGETResp sendMLSMaxPacket(String zHost, int zPort, MiniData zMaxMessage) throws IOException {
		...
		//Tell the NIO
		SELFSystem.getInstance().getNIOManager().getTrafficListener().addWriteBytes("sendMLSMaxPacket",zMaxMessage.getLength());
		...
		//Tell the NIO
		SELFSystem.getInstance().getNIOManager().getTrafficListener().addReadBytes("sendMLSMaxPacket",resp.getLength());
	...
	public static boolean checkValidSxAddress(String zMaximaAddress) {
		
		if(!zMaximaAddress.startsWith("Sx")) {
			return false;
		}
