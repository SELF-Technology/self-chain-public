...
public class maxmessage extends Command {
	...
	public String getFullHelp() {
		return "\nmaxmessage\n"
				...
				+ "\n"
				+ "You can create an unencrypted message bu omitting the publickey.\n"
				+ "\n"
				+ "Examples:\n"
				+ "\n"
				+ "maxmessage action:encrypt data:0xFFEEDD\n"
				+ "\n"
				+ "maxmessage action:encrypt data:0xFFEEDD publickey:0x/Sx..30819F300D0..\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		if(action.equals("encrypt")) {
			...
			//Who is signing..
			MiniData pubkey 	= SELFSystem.getInstance().getMaxima().getPublicKey();
			MiniData privkey 	= SELFSystem.getInstance().getMaxima().getPrivateKey();
			...
			try {
				...
				if(datstr.startsWith("0xFFFFFFFF")) {
					...
					MiniData privkey = getDataParam("privatekey",SELFSystem.getInstance().getMaxima().getPrivateKey());
