...
public class rpc extends Command {
	...
	public String getFullHelp() {
		return "\nrpc\n"
				...
				+ "\n"
				+ "The Deafult user is self. That has write mode access. You can add other rpc users with read access.\n"
				+ "\n"
				+ "enable:\n"
				...
				+ "rpc enable:true\n"
				+ "\n"
				+ "rpc enable:true ssl:true password:selfrpcpassword\n"
				+ "\n"
				+ "rpc enable:true password:self\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception{
		...
		if(existsParam("enable")) {
			...
			//Stop the Old
			SELFSystem.getInstance().getNetworkManager().stopRPC();
			Thread.sleep(1000);
			...
			//Now start or stop..
			if(enable) {
				SELFSystem.getInstance().getNetworkManager().startRPC();
			}else {
				SELFSystem.getInstance().getNetworkManager().stopRPC();
			}
			...
			if(action.equals("adduser")) {
				...
				if(!GeneralParams.RPC_AUTHENTICATE) {
					throw new CommandException("You must set a default RPC password (for user self) via -rpcpassword to add extra users");
				}
				
				String username = getParam("username");
				if(username.equals("self")) {
					throw new CommandException("Cannot add User self - this is the default RPC user.");
				}
		...
		//Get the Public Key..
		Certificate cert 	= SSLManager.getSSLKeyStore().getCertificate("SELF_NODE");
