...
public class keys extends Command {
	...
	public JSONObject runCommand() throws Exception{
		...
		if(action.equals("list")) {
			...
			resp.put("miniaddress", Address.makeSelfAddress(newaddress.getAddressData()));
			...
			for(KeyRow kr : keys) {
				...
				if(!pubk.isEqual(actualkey)) {
					SelfLogger.log("[!] INCORRECT Public key : "+pubk+" / "+actualkey);
					wrong++;
				}else {
					SelfLogger.log("CORRECT Public key : "+pubk);
					correct++;
				}
			}
	...
	public static boolean checkKey(String zPublicKey) {
		...
		KeyRow kr 		= SelfDB.getDB().getWallet().getKeyFromPublic(zPublicKey);
		
		//Check this keys private key is correct - hash(Seed+Mod)
		if(!SelfDB.getDB().getWallet().checkSingleKey(kr.getPrivateKey(), kr.getModifier())) {
			return false;
		}
