...
public class history extends Command {
	...
	public JSONObject runCommand() throws Exception{
		...
		if(action.equals("size")) {
			...
			if(relevant) {
				int size = SelfDB.getDB().getTxPoWDB().getSQLDB().getRelevantSize();
				resp.put("size", size);
				...
				int size 			= SelfDB.getDB().getTxPoWDB().getSQLDB().getLatestTxPoWSize(starttime);
			...
			int size = SelfDB.getDB().getTxPoWDB().getSQLDB().customSizeQuery(where);
			resp.put("size", size);
			...
			if(relevant) {
				txps = SelfDB.getDB().getTxPoWDB().getSQLDB().getAllRelevant(max,offset);
			}else {
				txps = SelfDB.getDB().getTxPoWDB().getSQLDB().getLatestTxPoW(max,offset);
			}
