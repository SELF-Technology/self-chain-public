package org.self.system.commands.base;

import java.util.ArrayList;
import java.util.Arrays;

import org.self.system.commands.Command;
import org.self.system.network.self.NIOClient;
import org.self.system.network.self.NIOServer;
import org.self.utils.json.JSONObject;
import org.self.utils.messages.MessageProcessor;

public class trace extends Command {

	public trace() {
		super("trace","[enable:true|false] (filter:) (network:) - Show the message stacks of the internal Self Engine with optional filter string. Only works on terminal.");
	}
	
	@Override
	public String getFullHelp() {
		return "\ntrace\n"
				+ "\n"
				+ "Show the message stacks of the internal Self Engine with optional filter string.\n"
				+ "\n"
				+ "enable:\n"
				+ "    true or false, true to enable or false to disable.\n"
				+ "\n"
				+ "filter: (optional)\n"
				+ "    A case sensitive string to filter the messages by.\n"
				+ "\n"
				+ "network: (optional)\n"
				+ "    true or false, show low level network messages\n"
				+ "\n"
				+ "Examples:\n"
				+ "\n"
				+ "trace enable:true\n"
				+ "\n"
				+ "trace enable:true filter:MAIN\n"
				+ "\n"
				+ "trace enable:true filter:MINER\n"
				+ "\n"
				+ "trace enable:true filter:MDS\n"
				+ "\n"
				+ "trace enable:true filter:MDS network:true\n"
				+ "\n"
				+ "trace enable:true filter:NOTIFYMANAGER\n"
				+ "\n"
				+ "trace enable:true filter:TXPOWPROCESSOR\n"
				+ "\n"
				+ "trace enable:false\n";
	}
	
	@Override
	public ArrayList<String> getValidParams(){
		return new ArrayList<>(Arrays.asList(new String[]{"enable","filter","network"}));
	}
	
	@Override
	public JSONObject runCommand() throws Exception {
		JSONObject ret = getJSONReply();
		
		String act = (String)getParams().get("enable");
		if(act == null) {
			throw new Exception("Must specify enable");
		}
	
		boolean on = false;
		if(act.equals("true")) {
			on = true;
		}
	
		String filter = getParam("filter", "");
		
		MessageProcessor.setTrace(on, filter);
		
		boolean  network = getBooleanParam("network", false);
		NIOClient.mTraceON = network;
		NIOServer.mTraceON = network;
	
		JSONObject tr = new JSONObject();
		tr.put("enabled", on);
		tr.put("filter", filter);
		tr.put("shownetwork", network);
		
		ret.put("response", tr);
		
		return ret;
	}

	@Override
	public Command getFunction() {
		return new trace();
	}

}
