...
public class balance extends Command {
	...
	public balance() {
		super("balance","(address:) (tokenid:) (confirmations:) (megammr:) - Show your total balance of Self and tokens");
	}
	...
	public String getFullHelp() {
		return "\nbalance\n"
				+ "\n"
				+ "Show your total balance of Self and tokens.\n"
				+ "\n"
				+ "address: (optional)\n"
				+ "    Show the balance for a specific 0x or Sx address.\n"
				+ "\n"
				+ "tokenid: (optional)\n"
				+ "    Show the balance for a specific tokenid. Self is 0x00.\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception{
		...
		MiniNumber confirmations 	= getNumberParam("confirmations", GlobalParams.SELF_CONFIRM_DEPTH);
		...
		//Always show a Self Balance
		alltokens.add(Token.TOKENID_SELF.to0xString());
		totalcoins.put(Token.TOKENID_SELF.to0xString(), MiniNumber.ZERO);
		
		if(debug) {
			SelfLogger.log("List of found relevant coins");
		}
		...
		for(Coin coin : coins) {
			...
			if(debug) {
				SelfLogger.log("Coin : "+coin.toJSON().toString());
			}
			...
			if(debug) {
				...
				if(!simple) {
					SelfLogger.log("NON-SENDABLE : "+coin.toJSON().toString());
				}else {
					SelfLogger.log("SENDABLE : "+coin.toJSON().toString());
				}
			}
