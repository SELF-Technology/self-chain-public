...
//    	SelfLogger.log("PEERSCHECKER:"+zMessage.toString());
            ...
            if (force || P2PFunctions.getAllConnectedConnections().size() > 0) {
                ...
                if (greet != null) {
                    ...
                    if(validversion) {
                    		...
                    		if(tip != null) {
                    			...
                    			if(tip.getTxPoW().getBlockNumber().isLess(new MiniNumber(block))) {
                    				if(forcelog || GeneralParams.PEERSCHECKER_lOG) {
                        				SelfLogger.log("[-] Can't check peer as tip below check block tip:"+tip.getTxPoW().getBlockNumber().toString()+" check:"+block);
                        			}	
                    			}else {
	                    			...
	                    			if(checknode != null) {
	    	                			...
	    	                			if(!checknode.getTxBlock().getTxPoW().getTxPoWID().equals(blockhash)) {
	    	                				...
	    	                				if(forcelog || GeneralParams.PEERSCHECKER_lOG) {
	    	                					SelfLogger.log("PEERS CHECKER incorrect chain! @ "+block+" "+address.toString()+" ");
	    	                				}
	                    				...
	                    				if(forcelog || GeneralParams.PEERSCHECKER_lOG) {
	                    					SelfLogger.log("PEERS CHECKER incorrect chain! ( null checknode ) @ "+block+" "+address.toString()+" ");
	                    				}
                    			...
                    			if(forcelog || GeneralParams.PEERSCHECKER_lOG) {
                    				SelfLogger.log("[-] Can't check peer as we have no block data");
                    			}
                            }
                    	}else {
                    		if(forcelog || GeneralParams.PEERSCHECKER_lOG) {
                    			SelfLogger.log("PEERS CHECKER no block data @ "+address.toString());
                    		}
                    	...
                    	if(validversion) {
                    		if(forcelog) {
                    			SelfLogger.log("FORCECHECK - VALID CHAIN "+address.toString());
                    		}
                    	}
                    }
                }else {
                	if(forcelog) {
            			SelfLogger.log("NULL greeting from Peer "+address.toString());
            		}
                }
...
//              	P2PFunctions.log_debug("[!] P2P not connected to internet - try again in 60 seconds");
        	...
        	if(P2PFunctions.isNetAvailable()) {
        		...
        		SelfLogger.log("No Network connection - cannot perform peers check");
        	}
        }
