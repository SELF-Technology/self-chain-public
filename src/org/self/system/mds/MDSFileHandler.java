...
import org.self.system.mds.multipart.MultipartData;
import org.self.system.mds.multipart.MultipartParser;
import org.self.system.mds.polling.PollStack;
import org.self.system.mds.publicmds.MDSPublicPage;
...
public class MDSFileHandler implements Runnable {
	...
	public static String SELF_DOWNLOAD_AS_FILE = "_self_download_as_file_";
	...
	public void run() {
		...
		try {
	        ...
	        bufferedReader 	= new BufferedReader(new InputStreamReader(inputStream, MiniString.SELF_CHARSET));
			...
			if(GeneralParams.SHOW_NETWORK_CALLS) {
				if(GeneralParams.SHOW_NETWORK_POLLS) {
					SelfLogger.log("Network Call : "+fileRequested,false);
				}else {
			...
			if(	fileRequested.startsWith("mdscommand_/") ) {
				...
				//The final data
				byte[] fdata = result.getBytes(MiniString.SELF_CHARSET);
				...
				dos.writeBytes("Server: Self MDS Command server 1.3\r\n");
				...
				SELFSystem.getInstance().getMDSManager().PostMessage(MDSManager.MDS_MINIDAPPS_RESETSESSIONS);
				...
				String minihubuid = SELFSystem.getInstance().getMDSManager().getDefaultMiniHUB();
				...
				if(!SelfDB.getDB().getUserDB().getPublicMDS()) {
					SelfLogger.log("Access forbidden : Public MDS site disabled..!");
				...
				if(webfile.getName().equalsIgnoreCase("mds.js")) {
					...
					if(!webfile.exists() || !ischild || webfile.isDirectory()) {
						...
						if(!webfile.getAbsolutePath().endsWith(".map")) {
							SelfLogger.log("HTTP : unknown file requested "+fileRequested+" "+webfile.getAbsolutePath());
				    	}
			    		...
			    		//SelfLogger.log("File Requested : "+fileRequested,false);
			    		
			    		boolean downloader 	= false;
			    		String filename 	= webfile.getName();
			    		if(filename.contains(SELF_DOWNLOAD_AS_FILE)){
			    			//Remove the ending..
			    			filename 	= filename.replace(SELF_DOWNLOAD_AS_FILE, "");
			    			downloader 	= true;
			    		}
			...
			SelfLogger.log("MDSCommandException : "+exc.toString());
			
			// send HTTP Headers
			try {
				dos.writeBytes("HTTP/1.1 500 Internal Server Error\r\n");
				dos.writeBytes("Server: HTTP MDS Server from Self 1.3\r\n");
				dos.writeBytes("Date: " + new Date()+"\r\n");
				dos.writeBytes("Content-type: text/plain\r\n");
				dos.writeBytes("Access-Control-Allow-Origin: *\r\n");
				dos.writeBytes("\r\n"); // blank line between headers and content, very important !
			...
			if(lasterror > 120 * 1000) {
				SelfLogger.log("MDS Invalid SessionID Exception : You need to close your MiniDAPPS, logout of the MDS and then log back in..");
	...
	public String loadResouceFile(String zResource) throws IOException {
		...
		return new String(file, MiniString.SELF_CHARSET);
	}
}
