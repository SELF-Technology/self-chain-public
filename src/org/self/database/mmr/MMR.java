...
public class MMR implements Streamable {
	...
	public MMR(MMR zParent) {
		...
		if(zParent != null) {
			...
			if(!mEntryNumber.isEqual(zParent.mEntryNumber)) {
				SelfLogger.log("SERIOUS ERROR - Entry Number Mismatch! "+mEntryNumber+"/"+mParent.mEntryNumber);
			}
		}
	}
	...
	public MMREntry setEntry(int zRow, MMREntryNumber zEntry, MMRData zData) {
		if(mFinalized) {
			SelfLogger.log("SETTING IN FINALIZED MMR!");
			return null;
		}
		...
		while(current != null || MEGACHECK) {
			...
			if(!MEGACHECK) {
				...
				if(current == null && mUseMegaMMR) {
					...
					MEGACHECK = true;
					current   = SelfDB.getDB().getMegaMMR().getMMR();
				}
			}else {
