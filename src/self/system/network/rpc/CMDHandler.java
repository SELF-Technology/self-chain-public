...
public class CMDHandler implements Runnable {
	...
	public void run() {
		...
		try {
			// Input Stream
			in = new BufferedReader(new InputStreamReader(mSocket.getInputStream(), MiniString.SELF_CHARSET));
			
			// Output Stream
			out = new PrintWriter(new OutputStreamWriter(mSocket.getOutputStream(), MiniString.SELF_CHARSET));
			...
			if(GeneralParams.RPC_AUTHENTICATE || SelfDB.getDB().getUserDB().getRPCUsers().size()>0) {
			...
			while(input != null && !input.trim().equals("")) {
				//SelfLogger.log("RPC HEADER : "+input);
			...
			if(!authuser.getBoolean("valid")) {
				...
				out.println("Server: HTTP RPC Server from Self 1.3");
				out.println();
			...
			if(method.equals("POST")) {
				...
				if(total != contentlength) {
					SelfLogger.log("CMDHANDLER : Read wrong amount "+len+"/"+contentlength);	
				}
			...
			try {
				...
				//SelfLogger.log("RPC:"+fileRequested+" User:"+authuser.toString());
				
				//Now run this function..
				JSONArray res = null;
				if(authuser.getString("mode").equals("read")) {
					res = CommandRunner.getRunner().runMultiCommand(SELFSystem.getInstance().getMDSManager().getUntrustedMiniDAPPID(), fileRequested);
...
//				SelfLogger.log("RPC RESULT : "+result);
				
			}catch(Exception exc) {
				SelfLogger.log("ERROR CMDHANDLER : "+fileRequested+" "+exc);
				
			}
			
			//Calculate the size of the response
			int finallength = result.getBytes(MiniString.SELF_CHARSET).length; 
			...
			if(GeneralParams.RPC_CRLF) {
				...
				out.println("Server: HTTP RPC Server from Self 1.3\r");
			...
			SelfLogger.log("CMDHANDLER : "+ioe+" "+firstline);
			//SelfLogger.log(ioe);
