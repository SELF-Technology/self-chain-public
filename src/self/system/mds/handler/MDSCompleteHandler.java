...
public class MDSCompleteHandler implements Runnable {
	...
	public void run() {
		...
		try {
			...
			//SelfLogger.log(fileRequested,false);
			...
			if(!method.equals("POST") || uid.equals("")) {
				...
				if(command.equals("sql")) {
					...
					SelfLogger.log("ERROR MDS COMPLETE HANDLER REQ : "+command+" "+params);
				...
				out.println("Server: HTTP SQL Server from Self 1.3");
			...
			SelfLogger.log("MDSCompleteHandler : "+ioe.toString());
			outputError(out);
...
//				SelfLogger.log(e);
	...
	private static void outputError(PrintWriter zOut) {
		...
		zOut.println("Server: HTTP MDS Server from Self 1.3");
