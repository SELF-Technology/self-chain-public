...
import org.self.system.mds.runnable.api.APIService;
...
public class MDSJS {
	...
	public void log(String zMessage, boolean zNotifyAll) {
		if(!zNotifyAll) {
			SelfLogger.log("MDS_"+mMiniDAPPName+"_"+mMiniDAPPID+" > [NOTIFY:"+zNotifyAll+"] "+zMessage, zNotifyAll);
		}else {
			SelfLogger.log("MDS_"+mMiniDAPPName+"_"+mMiniDAPPID+" > "+zMessage, zNotifyAll);
		}
	}
	...
	public void load(String zFile) {
		...
		try {
			...
			//File not found..
			SelfLogger.log(e.toString());
			return;
		}
		
		//Convert to CODE..
		String code = new String(codedata, MiniString.SELF_CHARSET);
