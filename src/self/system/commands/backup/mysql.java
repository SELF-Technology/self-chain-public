...
public class mysql extends Command {
	...
	public String getFullHelp() {
		return "\nmysql\n"
				...
				+ "mysql action:update\n"
				+ "\n"
				+ "mysql action:addresscheck address:SxG08.. \n"
				+ "\n"
				+ "mysql action:resync\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		if(action.equals("info")) {
			...
			boolean autobackup = SelfDB.getDB().getUserDB().getAutoBackupMySQL();
			resp.put("autobackup", autobackup);
			
			boolean logindetails = SelfDB.getDB().getUserDB().getAutoLoginDetailsMySQL();
			...
			while(true) {
				if(logs) {
					SelfLogger.log("MySQL Verifying from : "+startload, false);
				}
			...
			if(mysqlfirstblock == -1) {
				...
				if(lastarch != 0 || lastarch != 1) {
					SelfLogger.log("Archive NOT starting from 0!.. Load from start of archive @ "+lastarch);
					startload = lastarch; 
				}
			}
			...
			while(!finished){
				...
				if(logs) {
					SelfLogger.log("MySQL block transfer from "+startload+" to "+(ender-1));
				}
				...
				if(blocks.size()==0) {
					//All blocks injected.. 
					SelfLogger.log("All blocks added");
					finished = true;
					break;
				}
...
//					NotifyListener(selflistener,"Loading "+start.getTxPoW().getBlockNumber()+" @ "+new Date(start.getTxPoW().getTimeMilli().getAsLong()).toString());
				...
				if(error) {
					SelfLogger.log("ERROR : There was an error processing that IBD");
					break;
				}
...
//			NotifyListener(selflistener,"All blocks loaded.. pls wait");
			...
			//And NOW shut down..
			//SELFSystem.getInstance().getTxPoWProcessor().stopMessageProcessor();
			...
			//And now restore..
			SelfLogger.log("Restoring H2 Archive DB..");
			...
			SelfLogger.log("First block:"+mysqllastblock);
			SelfLogger.log("Last block:"+mysqlfirstblock);
			...
			while(true) {
				if(logs) {
					SelfLogger.log("Loading from H2 @ "+startload);
				}
				...
				for(TxBlock block : blocks) {
					...
					//Send to H2
					mysql.saveBlock(block);
					//SelfLogger.log("Save block : "+block.getTxPoW().getBlockNumber());
			...
			while(true) {
				if(logs) {
					SelfLogger.log("Loading from MySQL @ "+startload);
				}
				ArrayList<TxBlock> blocks = mysql.loadBlockRange(new MiniNumber(startload));
				if(blocks.size()==0) {
					//All blocks checked
			...
			if(casc!=null) {
				...
				SelfLogger.log("No cascade found in MySQL..");
			}
			
			//And Now export to File..
			SelfLogger.log("Exporting to H2 SQL file..");
			...
			Cascade casc = mysql.loadCascade();
			if(casc!=null) {
				SelfLogger.log("Cascade found in MySQL..");
			...
			//How many entries..
			int total = mysql.getCount();
			SelfLogger.log("Total records found : "+total);
			...
			SELFSystem.getInstance().MYSQL_IMPORTING_NO_ACTION = true;
			...
			try {
				...
				while(true) {
					...
					if(logs) {
						if(counter % 10 ==0) {
							if(blocks.size()>0) {
								SelfLogger.log("Loading from RAW Block : "+blocks.get(0).getTxPoW().getBlockNumber());
							}
						}
					}
				...
				SELFSystem.getInstance().MYSQL_IMPORTING_NO_ACTION = false;
			...
			while(true) {
				...
				if(logs) {
					if(counter % 10 ==0) {
						if(blocks.size()>0) {
							SelfLogger.log("Loading from RAW Block : "+blocks.get(0).getTxPoW().getBlockNumber()+" SAVING:"+savingblocks,false);
						}
					}
				}
				...
				for(TxBlock block : blocks) {
					...
					if(blocknum > startchecking) {
						...
						if(txblk == null) {
							mysql.saveBlock(block);
							totalsaved++;
							SelfLogger.log("Save MISSING block : "+blocknum);
						}
					}
				}
	...
	public static void convertMySQLParams(String zMySQLDB) {
		...
		try {
			...
			SelfLogger.log("MYSQL Database Setup : "+username+":***@"+dbhost+" / "+dbname);
			...
			if(GeneralParams.MYSQL_DB_DELAY != 0) {
				SelfLogger.log("Waiting "+GeneralParams.MYSQL_DB_DELAY+" ms before attempting first MySQL connection..");
			...
			//Failed..
			SelfLogger.log("Failed to connect to MySQL DB - MUST be username:password@host:port/database "+exc.toString());
		}
	}
	...
	public static void main(String[] zArgs) {
		...
		MySQLConnect conn = new MySQLConnect("127.0.0.1:3306", "selfdb", "selfuser", "selfpassword");
		try {
