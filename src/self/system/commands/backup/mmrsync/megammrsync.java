...
import org.self.database.wallet.KeyRow;
...
public class megammrsync extends Command {
	...
	public JSONObject runCommand() throws Exception {
		...
		if(action.equals("mydetails")) {
			...
			Wallet wal = SelfDB.getDB().getWallet();
			...
			if(ibdtest == null) {
				...
				SelfLogger.log("Check Connection passed!");
			}
			...
			SELFSystem.getInstance().getTxPoWProcessor().postProcessIBD(mibd.getIBD(), "0x00", true);
			...
			SelfLogger.log("Import CoinProofs.. "+mibd.getAllCoinProofs().size());
			...
			SelfLogger.log("Peers List : "+mibd.getPeersList());
	...
	public void updateP2PDB(String zPeersList) {
    	...
    	while(strtok.hasMoreTokens()) {
    		...
    		try {
    			...
    			SelfLogger.log("Invalid peer in MegaMMR peersList : "+peer);
    		}
    	}
    	
    	//Get the Base Folder
    	File basefolder = SelfDB.getDB().getBaseDBFolder();
	...
	public static ArrayList<Coin> searchMegaCoins(
		...
		while(tip!=null || MEGACHECK) {
			...
			if(!MEGACHECK) {
				...
				coins = new ArrayList<Coin>(SelfDB.getDB().getMegaMMR().getAllCoins().values());
			}
	...
	public static MegaMMRIBD sendMegaMMRSyncReq(String zHost, int zPort, MegaMMRSyncData zSyncData) {
		...
		try {
			...
			//Tell the NIO
			SELFSystem.getInstance().getNIOManager().getTrafficListener().addWriteBytes("sendMegaMMRSyncReq",msg.getLength());
			
			//Load the message
			MiniData resp = MiniData.ReadFromStream(dis);
			
			//Tell the NIO
			SELFSystem.getInstance().getNIOManager().getTrafficListener().addReadBytes("sendMegaMMRSyncReq",resp.getLength());
			...
			SelfLogger.log("MegaMMR Sync connection : "+exc+" @ "+zHost+":"+zPort);
