...
public class mysqlcoins extends Command {
	...
	public String getFullHelp() {
		return "\nmysqlcoins\n"
				...
				+ "\n"
				+ "mysqlcoins action:search address:Sx87DE..\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		if(action.equals("info")) {
			...
			boolean autobackup = SelfDB.getDB().getUserDB().getAutoBackupMySQLCoins();
			...
			SelfLogger.log("Updating CoinsDB..");
			...
			if(lastcoin != -1) {
				SelfLogger.log("Last coin found @ "+lastcoin);
				mysqlstartblock = lastcoin+1;
			}
			SelfLogger.log("Starting from : "+mysqlstartblock);
			...
			while(true && !maxblocksreached) {
				...
				for(TxBlock block : blocks) {
					...
					String date 	= SelfLogger.DATEFORMAT.format(new Date(timemilli));
					...
					for(CoinProof cc : inputs) {
						...
						if(logs) {
							SelfLogger.log("Found Input coin @ "+blocknum+" : "+cc.getCoin().toJSON().toString());
						}
					...
					for(Coin cc : outputs) {
						...
						if(logs) {
							SelfLogger.log("Found Output coin @ "+blocknum+" : "+cc.toJSON().toString());
						}
					...
					if(coincounter>maxcoins) {
						SelfLogger.log("Max coins added : "+coincounter+"/"+maxcoins);
						maxblocksreached=true;
						break;
					}
				}
