...
import org.self.system.network.maxima.MaxMsgHandler;
...
public class maxima extends Command {
	...
	public String getFullHelp() {
		return "\nmaxima\n"
				...
				+ "\n"
				+ "Maxima is an information transport layer running on top of Self.\n"
				+ "\n"
				+ "action:\n"
				...
				+ "\n"
				+ "maxima action:send to:SxG18H.. application:appname data:0xFED5..\n"
				+ "\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		MaximaManager max = SELFSystem.getInstance().getMaxima();
		...
		if(func.equals("info")) {
			...
			String fullhost = GeneralParams.SELF_HOST+":"+GeneralParams.SELF_PORT;
			
			//Show details
			details.put("name", SelfDB.getDB().getUserDB().getMaximaName());
			details.put("icon", SelfDB.getDB().getUserDB().getMaximaIcon());
			
			details.put("publickey", max.getPublicKey().to0xString());
			details.put("sxpublickey", Address.makeSelfAddress(max.getPublicKey()));
			...
			SelfDB.getDB().getUserDB().setMaximaName(name);
			...
			SelfDB.getDB().getUserDB().setMaximaIcon(icon);
			SelfDB.getDB().saveUserDB();
...
//			ret.put("response", details);
			...
			if(existsParam("to")) {
				...
				if(fullto.startsWith("MAX#")) {
					...
					if((boolean)maxaddress.get("success")) {
						...
						SelfLogger.log("ATTEMPT SEND MAX CONTACT : "+fullto);
				...
				if(!maxextra.checkValidSxAddress(fullto)) {
	...
	public static Message createSendMessage(String zFullTo, String zApplication, MiniData zData) {
		...
		if(publickey.startsWith("Sx")) {
			publickey = Address.convertSelfAddress(publickey).to0xString();
		}
