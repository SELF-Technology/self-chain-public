...
import org.self.system.mds.handler.CMDcommand;
...
public class mds extends Command {
	...
	public String getFullHelp() {
		return "\nmds\n"
				...
				+ "\n"
				+ "file: (optional)\n"
				+ "    The file name of the MiniDapp to install. Can either be in the Self folder or specify the file path.\n"
				+ "\n"
				+ "uid (optional)\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		if(action.equals("list")) {
			...
			MDSManager mdsman = SELFSystem.getInstance().getMDSManager();
			...
			mds.put("connect", "https://"+GeneralParams.SELF_HOST+":"+GeneralParams.MDSFILE_PORT);
			mds.put("password", SELFSystem.getInstance().getMDSManager().getMiniHUBPasword());
			
			mds.put("publicmds", SelfDB.getDB().getUserDB().getPublicMDS());
			mds.put("publicmdsuid", SELFSystem.getInstance().getMDSManager().getPublicMiniDAPPSessionID());
			mds.put("untrustedmdsuid", SELFSystem.getInstance().getMDSManager().getUntrustedMiniDAPPSessionID());
			...
			PendingCommand pending = SELFSystem.getInstance().getMDSManager().getPendingCommand(uid);
			String minidappid = "";
			if(pending!=null) {
				...
				SELFSystem.getInstance().getMDSManager().removePending(uid);
			...
			File dest 	= new File( SELFSystem.getInstance().getMDSManager().getWebFolder() , rand);
			if(dest.exists()) {
			...
			SelfDB.getDB().getUserDB().removeUninstalledMiniDAPP(md.getName());
			...
			copyfolder.mkdirs();
			File minisharefile 	= SELFSystem.getInstance().getMDSManager().getMiniDAPPShareFile(md);
			...
			SELFSystem.getInstance().getMDSManager().PostMessage(installed);
			...
			//Get the Minidapp..
			File minisharefile 	= SELFSystem.getInstance().getMDSManager().getMiniDAPPShareFile(uid);
			...
			//Log it.. 
			//SelfLogger.log("MDS Share Dapp : "+minisharefile.getAbsolutePath()+" to "+copyto.getAbsolutePath());
			//SelfLogger.log("Parent : "+copyto.getParentFile());
			...
			String minihub = SelfDB.getDB().getUserDB().getDefaultMiniHUB();
			if(uid.equals(minihub)) {
				throw new CommandException("Cannot delete the MiniHUB");
			}
			
			//Add to uninstalled..
			MiniDAPP md = SELFSystem.getInstance().getMDSManager().getMiniDAPP(uid);
			if(md == null) {
				throw new CommandException("MiniDAPP not found");
			}
			
			SelfDB.getDB().getUserDB().addUninstalledMiniDAPP(md.getName());
			SelfDB.getDB().saveUserDB();
			...
			// Delete web..
			String mdsroot 	= SELFSystem.getInstance().getMDSManager().getRootMDSFolder().getAbsolutePath();
			File dest 		= SELFSystem.getInstance().getMDSManager().getWebFolder();
			...
			//Delete Data folder
			SELFSystem.getInstance().getMDSManager().shutdownSQL(uid);
			File dbfolder1 = SELFSystem.getInstance().getMDSManager().getMiniDAPPDataFolder(uid);
			if(dbfolder1.exists()) {
			...
			SELFSystem.getInstance().getMDSManager().PostMessage(uninstall);
			...
			String mdsroot 	= SELFSystem.getInstance().getMDSManager().getRootMDSFolder().getAbsolutePath();
			File minidapp 	= new File(SELFSystem.getInstance().getMDSManager().getWebFolder(),uid);
			if(minidapp.exists()) {
				MiniFile.deleteFileOrFolder(mdsroot, minidapp);
			}
			...
			MiniFile.deleteFileOrFolder(copyfolder.getAbsolutePath(), copyfolder);
			copyfolder.mkdirs();
			File minisharefile 	= SELFSystem.getInstance().getMDSManager().getMiniDAPPShareFile(newmd);
			...
			if(uid.equals(SELFSystem.getInstance().getMDSManager().getPublicMiniDAPPID())) {
				throw new CommandException("Cannot set WRITE permission for Public MiniDAPP");
			
			}else if(uid.equals(SELFSystem.getInstance().getMDSManager().getUntrustedMiniDAPPID())) {
			...
			File root = SELFSystem.getInstance().getMDSManager().getWebFolder();
			...
			if(files != null) {
				...
				for(File dapp : files) {
					...
					if(mdorig != null) {
						...
						SelfLogger.log("EMPTY MiniDAPP folder deleted : "+dapp.getAbsolutePath());
			...
			//There has been a change
			SELFSystem.getInstance().getMDSManager().PostMessage(MDSManager.MDS_MINIDAPPS_RESETALL);
		
		}else if(action.equals("publicmds")) {
			
			boolean enable = getBooleanParam("enable");
			
			SelfDB.getDB().getUserDB().setPublicMDS(enable);
	...
	public void sendPendingResult(String zMiniDAPPID, String zPendinUID, boolean zAccept, boolean zStatus, JSONObject zResult) {
		...
		if(!zMiniDAPPID.equals("")) {
			...
			SELFSystem.getInstance().PostNotifyEvent("MDS_PENDING", res, zMiniDAPPID);
		}
	}
