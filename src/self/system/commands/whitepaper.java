...
public class whitepaper extends Command {
	...
	public static final String WP = "\n"
            ...
            + "Spartacus Rex\n"
            + "\n"
            + "Abstract. Self is a cryptocurrency designed to be ultra-compact. Designed to remain\n"
            ...
            + "send their transaction. Unlike Self, IOTA does not use a blockchain, instead, opting for a transaction DAG or\n"
            ...
            + "    Self uses a Tx-PoW blockchain, an idea first started with P2Pool[10], a protocol that allows multiple\n"
            ...
            + "then sum all of that up into single blocks. This way Self removes the miners but keeps the blockchain as the\n"
            + "single PoW-secured time-ordered list of events.\n"
            + "    Each transaction in Self is PoW mined. The process is very similar to searching for blocks on any other\n"
            ...
            + "    This type of system means that if there are no transactions then there can be no blocks. Self will need\n"
            + "to reach a critical mass of transactions to sustain a secure blockchain. The transaction rate will need to be very high.\n"
            + "Since all the security of Self is derived from the number of transactions, the more transactions there are, the\n"
            + "more secure the network. There will be a bootstrap period until Self processes enough transactions to secure\n"
            ...
            + "liveliness of the network and in preventing censorship attacks. All Self users perform both the validation and\n"
            ...
            + "Self has an innovative and unique solution.\n"
            + "-Replace fee-based security.\n"
            + "-Make layer 2 secure layer 1.\n"
            + "\n"
            + "    Self runs over a Peer-to-Peer (P2P) network called Maxima. Every user on the Self network is\n"
            + "connected to every other user. Maxima opens up this P2P backbone, via a simple network API, so that users can\n"
            + "transmit any data they like, not just Self transactions, to other individual Maxima users, point-to-point and not\n"
            ...
            + "    Even better, users of Maxima who have no Self, no tokens, send no transactions and have no interest in\n"
            ...
            + "communication.\n"
            + "    All Maxima users run Self. All messages pay PoW. All PoW secures Self.\n"
            + "\n"
            + "We define a :\n"
            + "-Self transaction / on-chain / flood-fill / does not scale.\n"
            ...
            + "happen.\n"
            + "    As more and more fee-paying traffic is generated, Maxima incentivises users to set up Self routers,\n"
            + "effectively a node with an external IP which other Self users can easily access, which is invaluable to the integrity\n"
            ...
            + "runs Self.\n"
            + "    Self provides value transfer. Maxima provides information transfer.\n"
            + "\n"
            + "4. The Pulse Network\n"
            + "    Both Self and Maxima require users to perform a small amount of PoW when sending messages. The\n"
            ...
            + "functioning correctly. But in a crypto network like Self far more information on the health and status of a node\n"
            + "can be transmitted in a PING message. Add some PoW to a PING message, and you create a Pulse.\n"
            + "    Every 10 minutes every Self user creates a valid Tx-PoW message, with its' current correct block details,\n"
            ...
            + "-Shows the peer is a functioning Self node, has a working network connection, is a valid router for\n"
            ...
            + "network.\n"
            + "    Self has a Pulse. No zombies allowed (nodes with no Pulse).\n"
            + "\n"
            ...
            + "    Self, a distributed PoW blockchain, does not incentivise the acquisition of hash power, since there is no\n"
            ...
            + "    On Self this is the Burn (which also serves multiple purposes). A Burn, when the outputs of a\n"
            + "transaction sum to less than the inputs, is equivalent to paying every other user a very small fee. Since burning'\n"
            + "reduces the total number of Self in circulation, since all coins are created at genesis, those coins that are left are\n"
            ...
            + "    The Burn in Self serves multiple purposes:\n"
            ...
            + "paid. Self is the same, but users mine transactions instead of emails.\n"
            + "Constructively, from the maelstrom of transactions fired across the Self network, a single time-ordered\n"
            + "interlocking chain of blocks emerges revealing the complete transaction history.\n"
            + "Self Consensus is driven by HashCash and Burn.\n"
            + "\n"
            + "7. Energy\n"
            ...
            + "    Self is compatible with the concept of Free Energy. Energy you do not even pay for, since you generate it\n"
            ...
            + "Self does not have a database (or big book) that stores all the Unspent Transaction Outputs, instead,\n"
            ...
            + "    Self goes MMR real-time. Each block commits to the current MMR state for that block. Each user sends\n"
            ...
            + "    Self operates a validatory network, like Bitcoin, rather than a computational network, like Ethereum[18].\n"
            ...
            + "networks centralize around larger more powerful nodes. Self must allow everyone to run a Complete node.\n"
            ...
            + "    Unlike Bitcoin, Self natively supports Tokens. Unlike Ethereum, no efficiency is sacrificed when processing\n"
            + "tokens. As far as the network is concerned, token transactions are the same as Self transactions, are stored in\n"
            + "the MMR Proof DB and do not increase storage requirements. You prove their existence like you prove your Self\n"
            + "holdings. Tokens can be created by colouring a certain fractional amount of Self. All scripts applicable to Self\n"
            + "are equally applicable to Tokens.\n"
            + "    Transactions on Self are similar to Bitcoin transactions, yet upgraded in functionality and power. They\n"
            ...
            + "Self Script. This entire transaction can then be signed by 1 or more Public Keys. When sending a transaction, a\n"
            ...
            + "Self uses a simple yet powerful scripting language. A script will return TRUE or FALSE as to whether an\n"
            ...
            + "    Self also includes simple state variables per transaction, accessible to input scripts, so that a sequence\n"
            ...
            + "Forks ever (there are no Soft Forks on Self as all users are also miners), you need to use Quantum Secure\n"
            + "algorithms.\n"
            + "    Self uses the SHA3 hash algorithm for Tx-PoW mining, block and transaction hashes, proof chains,\n"
            + "and signing or verifying data. All of the cryptographic security of Self is provided by hash functions. The scripting\n"
            ...
            + "one time use WOTS is 400-800 bytes. Self signatures are certainly large when compared to normal Bitcoin\n"
            ...
            + "\n"
            + "12. Block Size\n"
            + "    Blocks in Self are small. As small as a transaction. All transactions are potential blocks. Blocks are just\n"
            + "lucky transactions. Blocks only contain references to transactions (Compact blocks in Bitcoin), that have already\n"
            + "been sent across the network, rather than the entire transaction. Self also uses an adaptive block size. The\n"
            ...
            + "    Self uses a Cascading Chain that aggressively prunes almost all data, whilst keeping a record of total\n"
            ...
            + "    Block speed can be much faster on a GHOST[7] chain and so Self is aiming for a 50 second block time[6].\n"
            + "\n"
            ...
            + "Self), who neither validate nor construct, and are simply swept along, whether they agree or not.\n"
            ...
            + "The Self Protocol is complete from inception, containing all the scaling and functionality it will ever have or\n"
            ...
            + "    \n"
            + "    Decentralization: Above all else Self strives to be a completely decentralized platform. Every user on\n"
            ...
            + "    Security: Self is a Distributed PoW secured blockchain. The true PoWer of the Self Network comes\n"
            ...
            + "of Self is hash-based, and as such, Quantum Secure. All nodes are Complete, so that light client attacks, where a\n"
            ...
            + "    Scalability: The Self Protocol allows for both on-chain and off-chain transactions. The block size is\n"
            + "adaptive and can grow and shrink as the users see fit. Powerful off-chain protocols, some that already exist today,\n"
            + "take traffic off layer 1 and onto layer 2. Layer 2 allows near-limitless innovation and throughput. Self can utilise\n"
            + "current strategies like the Lightning Network and Sidechains, putting these abilities squarely in the hands of every\n"
            + "user.\n"
            + "    Sacrifice: In order to be runnable by all at all times, Self not only requires a large user base, but Self\n"
            + "also asks for more from its users. Participation is a requirement. Self utilises an MMR database, maximal pruning\n"
            ...
            + "Self users have all the tools required to interact in a completely decentralized way. All users can\n"
            ...
            + "\n"
            + "Acknowledgements: Paddy Cerri, Hugo Feiler, Barnaby Girling, Adam Feiler, The Self Team.\n"
            + "References\n"
	...
	public whitepaper() {
		super("whitepaper","Print out the Self White Paper");
	}
