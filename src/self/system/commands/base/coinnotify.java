...
public class coinnotify extends Command {
	...
	public String getFullHelp() {
		return "\ncoinnotify\n"
				...
				+ "\n"
				+ "coinnotify action:check address:Sx12ABGF56..\n";
	}
	...
	public JSONObject runCommand() throws Exception{
		...
		if(action.equals("add")) {
			SelfDB.getDB().addCoinNotify(addr);
		
		}else if(action.equals("remove")) {
			boolean found = SelfDB.getDB().removeCoinNotify(addr);
			resp.put("found", found);
			
		}else if(action.equals("check")) {
			boolean found = SelfDB.getDB().checkCoinNotify(addr);
			resp.put("found", found);
