...
public class consolidate extends Command {
	...
	public String getFullHelp() {
		return "\nconsolidate\n"
				...
				+ "\n"
				+ "tokenid:\n"
				+ "    The tokenid for Self or custom token to consolidate coins for. Self is 0x00.\n"
				+ "\n"
				+ "coinage: (optional)\n"
				...
				+ "\n"
				+ "burn: (optional)\n"
				+ "    Amount of Self to burn with the transaction.\n"
				+ "\n"
				+ "debug: (optional)\n"
	...
	public JSONObject runCommand() throws Exception {
		...
		for(Coin cc : relcoins) {
			...
			if(!currentaddress.equals(coinaddress)) {
				...
				if(totalsigs+1>MAX_SIGS) {
					if(debug) {
						SelfLogger.log("Consolidate - max sigs reached "+totalsigs);
					}
					break;
				}
			...
			if(debug) {
				SelfLogger.log("Consolidate - add coin "+coinamount+" totalcoins:"+totalcoins+"  totalsigs:"+totalsigs+" coinid:"+cc.getCoinID().to0xString());
			}
			
			//Do checks..
			if(totalcoins>=MAX_COINS) {
				if(debug) {
					SelfLogger.log("Consolidate - max coins reached "+totalcoins);
				}
				break;
			}
		}
		...
		if(existsParam("password")) {
			...
			//Hide the password
			if(debug) {
				SelfLogger.log("Consolidate command : "+command+" password:####");
			}
			...
			if(debug) {
				SelfLogger.log("Consolidate command : "+command);
			}
		}
