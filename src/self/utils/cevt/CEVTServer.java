...
public class CEVTServer {
	...
	public static void main(String[] zArgs) {
		...
		System.out.println("SELF Address    : "+CEVT_ADDRESS);
		...
		if(KJ_TOKENID.equals("")) {
			...
			System.out.println("MUST specify the CEVT SELF address!..");
			System.exit(1);
	    ...
	    while(mRunning) {
	    	...
	    	try {
	    		...
	    		SelfLogger.log("Found : "+len+" CEVT Request coin..");
	    		...
	    		for(int i=0;i<len;i++){
	    			...
	    			if(coinage.isLess(mMinCoinage)) {
	    				SelfLogger.log("[NOT OLD ENOUGH YET] Coin: "+i+" amount:"+tokenamount+" coinid:"+coinid+" coinage:"+coinage.toString());
		    			continue;
	    			}
	    			...
	    			SelfLogger.log("[VALID] coinid:"+coinid+" coinage:"+coinage.toString()+" amount:"+tokenamount.toString()+" terminal:"+dterminalid);
	    			...
	    			SelfLogger.log(response.toString());
		    		...
		    		if(ourcoin == null) {
		    			SelfLogger.log("[WARNING] Could not find single spendable large enough coin! (1)");
		    			continue;
		    		}
		    		...
		    		if(status) {
		    			SelfLogger.log("[SUCCESS SEND] : "+tokenamount+" address:"+useraddress);
		    		}else {
