package org.self.utils.bondserver;
...
public class BondServer {
	...
	public static final String BOND_ADDRESS = "SxG0861MPQ3ZQTM4GFTZ0UJA74Y48A4GDPYM1NTVKDTU0B34BFDV86G5A0PD21N";
	...
	public static void main(String[] zArgs) {
		...
		System.out.println("Welcome to the Self Bond Server");
		...
		try {
			...
			if((boolean)addscript.get("status") != true) {
				SelfLogger.log(addscript.toString());
				System.exit(1);
			}
...
//		}
	    ...
	    while(mRunning) {
	    	...
	    	try {
	    		...
	    		SelfLogger.log("Current block : "+block.toString());
	    		...
	    		//SelfLogger.log(MiniFormat.JSONPretty(allcoins));
	    		
	    		int len = allcoins.size();
	    		SelfLogger.log("Found : "+len+" Bond Request coin..");
	    		...
	    		for(int i=0;i<len;i++){
	    			...
	    			if(coinage.isLess(mMinCoinage)) {
	    				SelfLogger.log("[NOT OLD ENOUGH YET] Coin: "+i+" amount:"+coinamount+" coinid:"+coinid+" coinage:"+coinage.toString());
		    			continue;
	    			}else if(coinage.isMore(mMaxCoinage)) {
	    				SelfLogger.log("[COIN TOO OLD] Coin: "+i+" amount:"+coinamount+" coinid:"+coinid+" coinage:"+coinage.toString());
		    			continue;
	    			} 
		    		...
		    		if(rate == null) {
		    			SelfLogger.log("INAVLID RATE NULL Coin: "+i+" amount:"+coinamount+" coinid:"+coinid+" coinage:"+coinage.toString());
		    			continue;
		    		}
	    			...
	    			if(coinamount.isLess(mMin) || coinamount.isMore(mMax)) {
	    				SelfLogger.log("[INVALID] Coin amount is outside allowed range "+coinamount.toString()+" coinid:"+coinid);
		    			continue;
	    			}
	    			...
	    			SelfLogger.log("[VALID] coinid:"+coinid+" coinage:"+coinage.toString()+" amount:"+coinamount.toString()+" interest:"+reqamount.toString());
	    			
	    			//Spend it..
	    			String spendable 	 = "coins coinage:5 sendable:true tokenid:0x00 checkmempool:true";
		    		jsonres 			 = runSingleCommand(spendable);
		    		JSONArray spendcoins = (JSONArray) jsonres.get("response");
		    		
		    		SelfLogger.log("Found : "+spendcoins.size()+" spenable coins not in mempool.. ");
		    		
		    		JSONObject ourcoin = getCoin(spendcoins, reqamount);
		    		if(ourcoin == null) {
		    			SelfLogger.log("[WARNING] Could not find single large enough coin! "+reqamount.toString());
		    			continue;
		    		}
		    		...
		    		if(useraddress == null) {
		    			SelfLogger.log("INAVLID USER ADDRESS NULL Coin: "+i+" amount:"+coinamount+" coinid:"+coinid+" coinage:"+coinage.toString());
		    			continue;
		    		}
		    		...
		    		String fcaddress		= "SxG087AH0HPWAYJPQTQGYEMG03F1K2R1H43HVWYH19NB0RTW3SZWY7Q2F79810N"; 
		    		...
		    		if(duration == null) {
		    			SelfLogger.log("Invalid Rate amount! "+rate+" @ coindid:"+coinid);
		    			continue;
		    		}
...
//		    		}
		    		...
		    		if(status) {
		    			SelfLogger.log("[SUCCESS SEND] : "+totaltouser.toString()+" rate:"+rate+" address:"+fcusercaddress);
		    		}else {
		    			SelfLogger.log("[!] ERROR :");
	...
	public static JSONObject runSingleCommand(String zCommand) throws IOException, ParseException {
		...
		//URLEncode..
    	String input 	= URLEncoder.encode(zCommand, MiniString.SELF_CHARSET);
		String full 	= mHost+input;
		
		//Now run this function..
		String result = null;
    	if(mSSL) {
    		result = RPCClient.sendGETBasicAuthSSL(full, "self", mPassword, mSSLContext);
		}else{
    		result = RPCClient.sendGETBasicAuth(full,"self",mPassword);
    	}
