...
public class ScriptTokenizer {
	...
	public ArrayList<ScriptToken> tokenize() throws SelfParseException{
		...
		while(mPos<mLength) {
			...
			if(isWhiteSpace(nextchar)) {
				...
				if(!testchar.equals("<")) {
					throw new SelfParseException("Incorrect Token found @ "+mPos+" "+nextchar+testchar);
				}
				...
				if(allcommands.contains(uppercase)) {
					...
					if(!waslastspace) {
						throw new SelfParseException("Missing space before Command @ "+mPos+" "+uppercase);
					}
					...
					if(lowercase.length() > 32) {
						throw new SelfParseException("MAX Variable length is 32 @ "+mPos+" "+word);
					}
					...
					throw new SelfParseException("Incorrect Token found @ "+mPos+" "+word);
				}
			}
