				...
				if(greettip.isLess(myroot)) {
					...
					for(MiniData current : zGreeting.getChain()) {
						...
						if(counter % 20 == 0) {
							...
							//Look in DB
							found = SelfDB.getDB().getArchive().exists(current.to0xString());
					...
					if(found.isEqual(MiniNumber.MINUSONE)) {
						...
						if(size>0) {
							found = SelfDB.getDB().getArchive().exists(zGreeting.getChain().get(size-1).to0xString());
						}
					}
					...
					if(!found.isEqual(MiniNumber.MINUSONE)) {
						...
						if(!toobig) {
							...
							ArrayList<TxBlock> blocks = SelfDB.getDB().getArchive().loadBlockRange(found, myroot);
							...
							SelfLogger.log("Intersection found but User too far back to sync.. too many blocks:"+total+" max:"+MAX_BLOCKS_FOR_IBD);
							createCompleteIBD();
						}
...
//							SelfLogger.log("Intersection found but User too far back to sync.. too many blocks "+blocks.size()+" max:34000");
//							createCompleteIBD();
//						}else {
//							for(TxBlock block : blocks) {
//								mTxBlocks.add(0,block);
...
//						}
						...
						SelfLogger.log("No Archive blocks found to match New User.. ");
						createCompleteIBD();
					}
					
				}else if(greetroot.isMore(mytip)) {
					//We are behind their cascade! - Nothing to send!
					SelfLogger.log("We are Too old to sync new user! greetroot"+greetroot+" mytip:"+mytip);
					createCompleteIBD();
					...
					if(found) {
						...
						SelfLogger.log("[!] When creating IBD - No Crossover found whilst syncing with new node. They are on a different chain. Please check you are on the correct chain");
	...
	public void createCompleteIBD() throws IOException {
		...
		mCascade = SelfDB.getDB().getCascade().deepCopy();
	...
	public void createArchiveIBD(MiniNumber zFirstBlock) {
		createArchiveIBD(zFirstBlock, SelfDB.getDB().getArchive(), false);
	}
		...
		try {
			...
			if(!zUseLocal) {
				...
				if(rootblock.isEqual(MiniNumber.ONE) && reqblock.isEqual(MiniNumber.ZERO)) {
					SelfLogger.log("Root Tree Archive IBD - starts at genesis");
					reqblock = MiniNumber.ONE;
				}
				
				if(rootblock.isEqual(reqblock)) {
					
					SelfLogger.log("Archive request for main tree data @ "+reqblock);
					...
					if(lastadded.isEqual(reqblock)) {
						...
						SelfLogger.log("Archive main tree data error.. root:"+lastadded+" req:"+reqblock);
					}
				}
			}
	...
	public boolean checkValidData() {
		...
		if(hasCascadeWithBlocks()) {
			...
			if(getTxBlocks().size()==0) {
				...
				//Something wrong..
				SelfLogger.log("[!] Received INVALID IBD no blocks.. with a cascade");
				...
				if(!treestart.isEqual(casctip.increment())) {
					...
					//Something wrong..
					SelfLogger.log("[!] Received INVALID IBD with cascade tip:"+casctip+" and tree start:"+treestart);
				...
				if(!checkcascade) {
					SelfLogger.log("[!] INCONSISTENT Cascade received! length:"+getCascade().getLength());
					return false;
				}
			}
		}
	...
	public void writeDataStream(DataOutputStream zOut) throws IOException {
		...
		try {
			...
			oom.printStackTrace();
			SelfLogger.log("OUT OF MEMORY on IBD size:"+len);
		}
	}
	...
	public static boolean checkOurChainHeavier(IBD zIBD) throws IOException {
		...
		if(!heavier) {
			...
			if(found) {
				SelfLogger.log("[!] Crossover found on heavier chain : "+foundblockID);
			}else {
				SelfLogger.log("[!] NO crossover found on heavier chain..");
			}
			
			SelfLogger.log("YOUR  WEIGHT from Crossover : "+myweight);
			SelfLogger.log("THEIR WEIGHT from Crossover : "+theirweight);
		}
