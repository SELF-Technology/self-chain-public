...
public class MiniData implements Streamable {
	...
	public static int SELF_MAX_HASH_LENGTH 		= 64;
	
	/**
	 * 512 MB Max MiniData size
	 */
	public static int SELF_MAX_MINIDATA_LENGTH 	= 1024 * 1024 * 512;
	...
	public void readDataStream(DataInputStream zIn, int zSize) throws IOException {
		...
		if(len > SELF_MAX_MINIDATA_LENGTH) {
	...
	public void writeHashToStream(DataOutputStream zOut) throws IOException {
		if(mData.length > SELF_MAX_HASH_LENGTH) {
			throw new IOException("Write Error : HASH Length greater than "+SELF_MAX_HASH_LENGTH+"! "+mData.length);
		}
	...
	public void readHashFromStream(DataInputStream zIn) throws IOException {
		int len = zIn.readInt();
		if(len > SELF_MAX_HASH_LENGTH) {
			throw new IOException("Read Error : HASH Length greater than "+SELF_MAX_HASH_LENGTH+"! "+len);
		}else if(len<0) {
