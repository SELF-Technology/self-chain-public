...
public class TreeKey {
	...
	public Signature sign(MiniData zData) {
		...
		//Check range
		if(mUses >= mMaxUses) {
			SelfLogger.log("SERIOUS ERROR : MAX TREEKEYS USED @ "+mPublicKey);
			mUses = 0;
		}
		
		//Get the Correct Node path..
		ArrayList<Integer> nodes = baseConversion(mUses, mKeysPerLevel, mLevels);
//		SelfLogger.log("KEY TREE SIGN : "+mUses+" "+nodes.toString());
	...
	public boolean verify(MiniData zData, Signature zSignature) {
		...
		if(total>MAX_KEY_LEVELS) {
			SelfLogger.log("[!] INVALID KEY found with "+total+" levels MAX:"+MAX_KEY_LEVELS);
			return false;
		}
...
//		SelfLogger.log("DATA "+data.to0xString(32));
		...
		for(int i=0;i<maxsigs;i++) {
			...
			//And verify..
			SelfLogger.log("VERIFY : "+ktverify.verify(data, sig));
			timediff = System.currentTimeMillis() - timenow;
			System.out.println("verify time "+timediff);
		}
		
//		SelfLogger.log(convertBase(10, 16).toString());
		
//		SelfLogger.log(baseConversion(new MiniNumber(27), 29, 4).toString());
