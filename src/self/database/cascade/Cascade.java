package org.self.database.cascade;
	...
	public void cascadeChain() {
		...
		while(current != null) {
			...
			if(superlev>=casclevel) {
				...
				totlevel++;
				if(totlevel>=GlobalParams.SELF_CASCADE_LEVEL_NODES) {
					if(casclevel<GlobalParams.SELF_CASCADE_LEVELS-1) {
						casclevel++;
						totlevel = 0;
					}
				}
...
//			oldlevel = node.getLevel();
	...
	public static boolean checkCascadeCorrect(Cascade zCascade) {
		...
		if(cnode!=null) {
			if(cnode.getLevel()!=0) {
				SelfLogger.log("Cascade does not start at level 0.. "+cnode.getLevel());
				return false;
			}
		}
		...
		while(cnode!=null) {
			...
			if(clevel!=oldlevel) {
				...
				if(clevel!=oldlevel+1) {
					SelfLogger.log("NEXT level up is not a single increment @ clevel:"+clevel+" oldlevel:"+oldlevel);
					return false;
				}
			...
			//SelfLogger.log("Checking.. "+counter+" "+cnode.getLevel()+" "+txp.getTxPoWID()+" lastnode:"+lastnode);
			
			//Now check that all the parents are in the cascade..
			boolean foundzero = false;
			for(int i=clevel;i<GlobalParams.SELF_CASCADE_LEVELS;i++) {
				...
				if(sparent.equals("0x00")) {
					...
					if(foundzero) {
						//Should ALL be zero..
						SelfLogger.log("NON zero node found in cascade after first zero node.."+sparent+" @ slevel "+i+"/"+clevel+" counter:"+counter);
						return false;
					}
					...
					if(lastnode) {
						if(i>clevel) {
							if(!checkPastNodeExists(cnode, sparent)) {
								SelfLogger.log("Parent not found in cascade.. "+sparent+" @ slevel "+i+"/"+clevel+" counter:"+counter);
								return false;
							}
						}
					}else {
	...
	public static void main(String[] zArgs) {
		...
		File cfile = new File("C:\\Users\\spartacusrex\\.self\\1.0\\databases\\cascade.db");
